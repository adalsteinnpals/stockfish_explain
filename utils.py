    ...
    def get_evaluation_table(self, verbose=True):
        ...
        for i in range(start_idx, start_idx + offset + 14):
            ...
            for text_ in text_split:
                new_text += text_.replace('King safety', 'King_safety').replace('Passed pawns',
                                                                                'Passed_pawns').strip().split(' ')
            new_text = [x for x in new_text if x != '']
            t.append(new_text)

        df = pd.DataFrame(t,
                          columns=['Eval term', 'MG_white', 'EG_white', 'MG_black', 'EG_black', 'MG_total', 'EG_total'])
...
def get_nnue_eval_from_fen(fen, parameters = None):
    stockfish = stockfish_eval(path=config['STOCKFISH_PATH'],
...
class eval_class(nn.Module):
    def __init__(self,
        ...
        self.chosen_map, self.static_map = reduce_piece_map(self.map, list_pieces = self.pertub_pieces)
    ...
    def forward(self, x):
        ...
        return_value = torch.from_numpy(np.array(float(results['eval']['NNUE']))).unsqueeze(0).unsqueeze(0)
